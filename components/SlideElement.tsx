...
const SlideElement: React.FC<SlideElementProps> = ({
  ...
  const getIconComponent = (iconName: string) => {
    ...
    return icons[iconName] || Smile;
  };

  const renderContent = () => {
    const { type, content, subtype, style } = element;
    ...
    switch (type) {
      case 'text':
        return isEditing ? (
          ...
          <div
            className="w-full h-full flex items-center text-left resize-none outline-none cursor-text"
      ...
      case 'icon':
        ...
        return (
          <div className="flex items-center justify-center w-full h-full" style={style}>
            <IconComponent size={element.width * 0.8} />
          </div>
        );
